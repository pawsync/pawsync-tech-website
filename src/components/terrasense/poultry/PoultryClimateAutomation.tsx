...
const connected = [
  ...
  { icon: Droplets, label: "Humidity Sensors" },
  { icon: Wind, label: "Air Quality Sensors" },
  { icon: Fan, label: "Fans" },
  { icon: Flame, label: "Heaters" },
  { icon: Snowflake, label: "Cooling System" },
  { icon: Wind, label: "Ventilation" },
  { icon: AlertTriangle, label: "Alarm System" },
];

const rules = [
  {
    condition: "Temperature > Configured High Limit",
    action: "Increase Ventilation",
    extra: "Send Alert",
  },
  {
    condition: "Temperature < Configured Low Limit",
    action: "Activate Heating",
  },
  {
    condition: "Air Quality Reaches Configured Warning Level",
    action: "Increase Ventilation",
    extra: "Notify Operator",
  },
];
    ...
    <section aria-labelledby="poultry-climate-heading" className="bg-[var(--ts-navy)] py-16 sm:py-20">
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        <div className="mx-auto max-w-2xl text-center">
          <Eyebrow tone="dark">Climate Automation</Eyebrow>
          <h2 id="poultry-climate-heading" className="mt-3 font-[family-name:var(--font-manrope)] text-3xl font-bold tracking-tight text-white sm:text-4xl">
            Automated Poultry House Climate Control
          </h2>
          <p className="mt-4 text-lg text-white/60">
            One controller, connected to the sensors and equipment that keep
            house conditions in range.
          </p>
        </div>
        ...
        <div className="mt-12 flex flex-col items-center">
          ...
          <div className="flex items-center gap-2 rounded-2xl border border-[var(--ts-accent)]/30 bg-[var(--ts-accent)]/10 px-5 py-3">
            ...
            <span className="text-sm font-bold text-[var(--ts-accent)]">Poultry House Controller</span>
          </div>
          ...
          {rules.map((rule) => (
            <div key={rule.condition} className="rounded-2xl border border-white/10 bg-white/5 p-5 font-mono text-xs leading-relaxed text-[var(--ts-accent)]">
              <div className="flex items-start gap-2">
                ...
                <div>
                  <p><span className="text-white/40">IF</span> {rule.condition}</p>
                  <p className="mt-1.5"><span className="text-white/40">THEN</span> {rule.action}</p>
                  {rule.extra && (
                    <p className="mt-1.5"><span className="text-white/40">AND</span> {rule.extra}</p>
                  )}
                </div>
              </div>
            </div>
        ...
        <p className="mx-auto mt-6 flex max-w-2xl items-center justify-center gap-2 text-center text-xs text-white/40">
          ...
          Thresholds and control logic are configurable according to the
          farm&apos;s requirements.
        </p>
      </div>
    </section>
