...
const connectedSystems: { icon: LucideIcon; label: string }[] = [
  { icon: Thermometer, label: "Temperatur" },
  { icon: Droplets, label: "Luftfeuchtigkeit" },
  { icon: Wind, label: "Luftqualität" },
  { icon: Fan, label: "Belüftung" },
  { icon: Flame, label: "Heizung" },
  { icon: Snowflake, label: "Kühlung" },
  { icon: Lightbulb, label: "Beleuchtung" },
  { icon: Settings2, label: "Futter" },
  { icon: Waves, label: "Wasser" },
  { icon: Gauge, label: "Pumpen" },
  { icon: Fan, label: "Ventilatoren" },
  { icon: Cpu, label: "Motoren" },
  { icon: DoorOpen, label: "Türen" },
  { icon: PlugZap, label: "Stromüberwachung" },
  { icon: Siren, label: "Alarme" },
];

const ioLabels = [
  { label: "Sensoreingänge", pos: "top-4 left-4" },
  { label: "Relais-/Steuerausgänge", pos: "top-4 right-4" },
  ...
  return (
    <section aria-labelledby="poultry-controller-heading" className="mx-auto max-w-7xl px-4 py-16 sm:px-6 sm:py-20 lg:px-8">
      <div className="mx-auto max-w-2xl text-center">
        <Eyebrow>Zentraler Controller</Eyebrow>
        <h2 id="poultry-controller-heading" className="mt-3 font-[family-name:var(--font-manrope)] text-3xl font-bold tracking-tight text-[var(--ts-navy)] sm:text-4xl">
          Mehrere Hofsysteme über eine Plattform steuern
        </h2>
        <p className="mt-4 text-lg text-[var(--ts-gray)]">
          Ein Konzept für einen zentralen PawSync-Geflügel-Controller, der
          die Sensoren, Anlagen und Systeme eines Stalls oder Betriebs
          verbindet.
        </p>
      </div>
      ...
      <div className="mt-12 grid gap-10 lg:grid-cols-[1fr_1fr] lg:items-center lg:gap-12">
        ...
        <div className="relative mx-auto aspect-[4/3] w-full max-w-md overflow-hidden rounded-3xl border border-white/10 bg-[var(--ts-navy)] p-6 shadow-2xl">
          ...
          <div className="relative flex h-full w-full items-center justify-center rounded-2xl border border-white/10 bg-white/[0.03]">
            <div className="flex flex-col items-center gap-2 text-center">
              ...
              <span className="text-xs font-bold uppercase tracking-wide text-white">Geflügel-Controller</span>
              <div className="mt-1 flex gap-1.5">
                {["LTE / Mobilfunk", "Lokales Display", "Status-LEDs"].map((tag) => (
        ...
        {/* Connected systems */}
        <div>
          <p className="text-sm font-bold uppercase tracking-wide text-[var(--ts-green)]">Verbundene Systeme</p>
      ...
      <p className="mx-auto mt-8 max-w-2xl text-center text-xs text-[var(--ts-gray)]">
        <strong className="font-semibold text-[var(--ts-navy)]">Beispielhafte Systemarchitektur</strong> — dies
        veranschaulicht die Art von Plattform, die wir entwickeln und bauen
        können; es handelt sich um ein Produktkonzept, nicht um ein
        bestehendes, handelsübliches Produkt.
      </p>
    </section>
  );
}
